...
// vp8EntropyCost[p] is the cost in millibits×1024 of coding an event with
// probability p/256. Exact values copied from VP8EntropyCost[256] in
// libwebp/src/dsp/cost.c. Mismatched/approximate values cause trellis and
// adaptation to undercount coefficient encoding cost, retaining too many
// non-zero coefficients (kodim01/05 +13–20% file-size gap vs cwebp).
...
// vp8BitCost returns the cost in millibits×1024 of coding 'bit' with
// probability prob/256. Mirrors VP8BitCost() in libwebp/src/enc/cost_enc.h:
//
//	return !bit ? VP8EntropyCost[proba] : VP8EntropyCost[255 - proba];
//
// proba is a uint8_t in libwebp; callers must pass values in [0,255].
func vp8BitCost(bit int, prob int) int {
	// Clamp to uint8 range to mirror C's uint8_t storage.
	if prob < 0 {
		prob = 0
	}
	if prob > 255 {
